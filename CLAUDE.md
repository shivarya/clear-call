...
**P4 (Tier B "only my voice") — seam only.** `SuppressionEngine{OFF,DFN3,PERSONALIZED}` drives `NoiseSuppression`; `SpeakerConditionedProcessor`/`VoiceEnrollment` are documented stubs that compose DFN3 until the real speaker-conditioned model + on-device enrollment land. Settings has a General vs "Only my voice · beta" chooser.
